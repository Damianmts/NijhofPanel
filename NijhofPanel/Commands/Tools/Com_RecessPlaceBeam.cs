    ...
    public void Execute(UIApplication app)
    {
        try
        {
            ...
            using (var tx = new Transaction(doc, "Recess: Place in Beam"))
            {
                ...
                // - Selecteer Beam (DirectShape of ander element)
                ...
                else if (isDuct)
                {
                    ...
                    else if (shape == ConnectorProfileType.Rectangular)
                    {
                        ...
                        var symbol = FindFamilySymbol(doc, "NLRS_00.00_GM_LB_sparing recht", "funderingsparing")
                                     ?? throw new InvalidOperationException("Familie 'NLRS_00.00_GM_LB_sparing recht' met type 'funderingsparing' niet gevonden.");
                        if (!symbol.IsActive) symbol.Activate();
