...
public class Com_TagSwitchcode : IExternalEventHandler
{
    ...
    {
        var elementsWithSwitchcode = new FilteredElementCollector(doc)
            .OfCategory(category)
        ...
        return elementsWithSwitchcode
            .Where(el => !existingSwitchcodeTags.Any(tag =>
    ...
    {
        var existingTag = new FilteredElementCollector(doc, doc.ActiveView.Id)
            .OfClass(typeof(IndependentTag))
            .Cast<IndependentTag>()
            .FirstOrDefault(tag => tag.GetTypeId() == tagSymbol.Id &&
                                   tag.GetTaggedReferences().Any(r => r.ElementId == element.Id));
        if (existingTag != null)
            return;
        ...
        tag.LookupParameter("Parameter to display")?.Set("Switchcode");
    }
